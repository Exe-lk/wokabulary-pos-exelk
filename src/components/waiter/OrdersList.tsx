...
export default function OrdersList({ staffId }: OrdersListProps) {
  ...
  const handleBillSent = () => {
    // Update the order status to COMPLETED in local state
    if (billModal.order) {
      setOrders(prevOrders =>
        prevOrders.map(order =>
          order.id === billModal.order!.id ? { ...order, status: 'COMPLETED' } : order
        )
      );
    }
  };
              ...
              {order.status !== 'COMPLETED' && (
                <div className="p-4 border-t border-gray-200">
                  <div className="flex space-x-3">
                    ...
                    {/* Send Bill Button - Only for SERVED orders */}
                    {order.status === 'SERVED' && (
                      <button
